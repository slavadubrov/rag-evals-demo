...
- It's small. It can run on a laptop; download and embedding time depend on hardware and cache state.
...
- `data/golden/generation.jsonl`. Evidence excerpts only (`evidence_excerpt`), not reviewed answers. Labeled synthetic QA is separately committed under `data/fixtures/`.
...
Authorization stays fixed to the tenant of the chosen gold document. Only locale/domain search predicates are corrupted. Eligible gold contains only authorized documents. Corpus and qrels revisions are pinned in the loader and recorded in reports.

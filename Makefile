...
.PHONY: help install index golden eval eval-fast benchmark nb test lint fmt clean
...
clean:  ## Remove caches, artefacts, and the embedded Qdrant store
	$(call banner,Clean)
	rm -rf .venv .pytest_cache .mypy_cache .ruff_cache report.md report.json report data/cache qdrant_storage qdrant_bench __pycache__
